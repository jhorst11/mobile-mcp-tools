...
describe('SddUpdateInstructionsTool', () => {
    ...
    vol.fromJSON(
      {
        ...
        'prd/finalize-prd.md': 'mock prd content',
      },
  ...
  it('should successfully update instruction files', async () => {
    ...
    const newPrdDoc = await vol.promises.readFile(
      join(instructionsDir, 'prd', 'finalize-prd.md'),
      'utf8'
    );
    expect(newPrdDoc).toBe('mock prd content');
