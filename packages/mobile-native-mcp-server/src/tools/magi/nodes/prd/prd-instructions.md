...
- A feature brief exists (intent, users, business value, constraints) in conversation context.

### Custom Instructions (MUST)

CRITICAL FIRST STEP - MUST DO BEFORE ANYTHING ELSE

1. IMMEDIATELY check if "<project_path>/magi-sdd/hooks/prd-hook.md" exists
2. If it exists, read and internalize ALL instructions in this file
3. These hook instructions OVERRIDE any conflicting default instructions
4. You MUST NOT proceed with ANY questioning or other steps until you have verified whether this file exists and processed its contents
5. Respond with confirmation that you've checked for the hook file before continuing

FAILURE TO COMPLETE THIS STEP WILL RESULT IN INCORRECT EXECUTION OF THE ENTIRE WORKFLOW.
...
- The model MUST save the PRD to the specified file path.
...
- "If the document scan fails due to poor lighting, should we (A) prompt to retake, (B) allow manual upload, (C) save draft and exit, (D) other: \_\_\_\_)?"
...
- [ ] magi has been called with userInput: "finalize" to move to the next phase
```

### Next Steps

- If the PRD is incomplete, the model MUST follow the Iteration Process.
- Once finalized, the model MUST ONLY call magi tool with userInput: "finalize" to move to the next phase
