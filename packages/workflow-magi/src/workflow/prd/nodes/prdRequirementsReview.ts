...
export class PRDRequirementsReviewNode extends AbstractGuidanceNode<PRDState> {
  constructor(nodeExecutor?: NodeExecutor, logger?: Logger) {
    super('requirementsReview', nodeExecutor, logger);
  }
  ...
  execute = (state: PRDState): Partial<PRDState> => {
    ...
    const resultSchema = z.object({
      approvedRequirementIds: z.array(z.string()),
      rejectedRequirementIds: z.array(z.string()),
      modifications: z
        .array(
          z.object({
        ...
        .optional(),
      userIterationPreference: z.enum(['finalize', 'continue_iteration']),
    });

    const guidanceData: NodeGuidanceData = {
      nodeId: 'requirementsReview',
      taskPrompt: this.generateRequirementsReviewGuidance(requirementsPath),
      taskInput: {
      ...
      metadata: {
        nodeName: this.name,
        description: 'Review requirements with the user',
      },
    };
  ...
  private generateRequirementsReviewGuidance(requirementsPath: string): string {
    return `
