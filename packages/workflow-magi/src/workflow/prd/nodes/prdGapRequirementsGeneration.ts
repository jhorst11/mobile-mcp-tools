...
export class PRDGapRequirementsGenerationNode extends AbstractGuidanceNode<PRDState> {
  ...
  execute = (state: PRDState): Partial<PRDState> => {
    ...
    const resultSchema = z.object({
      updatedRequirementsMarkdown: z.string(),
    });

    const guidanceData: NodeGuidanceData = {
      nodeId: 'gapRequirementsGeneration',
      taskPrompt: this.generateGapRequirementsGuidance(
        featureBriefPath,
        requirementsPath,
        state.identifiedGaps || []
      ),
      taskInput: {
      ...
      metadata: {
        nodeName: this.name,
        description: 'Generate additional requirements based on identified gaps',
      },
    };
