...
import { PRD_UPDATE_TOOL } from '../../../tools/prd/magi-prd-update/metadata.js';
import { getMagiPath, writeMagiArtifact, MAGI_ARTIFACTS } from '../../../utils/magiDirectory.js';
import { PRD_REVIEW_TOOL } from '../../../tools/prd/magi-prd-review/metadata.js';
import z from 'zod';
...
export class PRDUpdateNode extends AbstractGuidanceNode<PRDState> {
  ...
  execute = (state: PRDState): Partial<PRDState> => {
    ...
    const guidanceData: NodeGuidanceData = {
      ...
      resultSchema: PRD_UPDATE_TOOL.resultSchema,
      metadata: {
        nodeName: this.name,
    ...
    const validatedResult = this.executeWithGuidance<typeof PRD_UPDATE_TOOL.resultSchema>(
      guidanceData
    );
  ...
  private generatePRDUpdateGuidance(
    prdFilePath: string,
    reviewResult: z.infer<typeof PRD_REVIEW_TOOL.resultSchema>
  ): string {
