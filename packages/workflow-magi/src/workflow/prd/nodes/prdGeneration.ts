...
export class PRDGenerationNode extends AbstractGuidanceNode<PRDState> {
  ...
  execute = (state: PRDState): Partial<PRDState> => {
    ...
    const guidanceData: NodeGuidanceData = {
      ...
      resultSchema: PRD_GENERATION_TOOL.resultSchema,
      metadata: {
        nodeName: this.name,
    ...
    const validatedResult = this.executeWithGuidance<typeof PRD_GENERATION_TOOL.resultSchema>(
      guidanceData
    );
  ...
  private generatePRDGenerationGuidance(featureBriefPath: string, requirementsPath: string): string {
    return `
