...
export class PRDFeatureBriefGenerationNode extends AbstractGuidanceNode<PRDState> {
  ...
  execute = (state: PRDState): Partial<PRDState> => {
    ...
    const resultSchema = z.object({
      recommendedFeatureId: z.string(),
      featureBriefMarkdown: z.string(),
    });

    const guidanceData: NodeGuidanceData = {
      nodeId: 'featureBriefGeneration',
      taskPrompt: this.generateFeatureBriefGuidance(state.userUtterance, currentFeatureIds),
      taskInput: {
      ...
      metadata: {
        nodeName: this.name,
        description: 'Generate a comprehensive feature brief from the user utterance',
      },
    };
  ...
  private generateFeatureBriefGuidance(userUtterance: string, currentFeatureIds: string[]): string {
    return `
# ROLE
