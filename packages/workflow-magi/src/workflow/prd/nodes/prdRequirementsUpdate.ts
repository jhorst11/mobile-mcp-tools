...
import { REQUIREMENTS_UPDATE_TOOL } from '../../../tools/prd/magi-prd-requirements-update/metadata.js';
import { getMagiPath, writeMagiArtifact, MAGI_ARTIFACTS } from '../../../utils/magiDirectory.js';
import { RequirementsReviewResult } from '../../../tools/prd/magi-prd-requirements-review/metadata.js';
...
export class PRDRequirementsUpdateNode extends AbstractGuidanceNode<PRDState> {
  ...
  execute = (state: PRDState): Partial<PRDState> => {
    ...
    const guidanceData: NodeGuidanceData = {
      ...
      resultSchema: REQUIREMENTS_UPDATE_TOOL.resultSchema,
      metadata: {
        nodeName: this.name,
    ...
    const validatedResult =
      this.executeWithGuidance<typeof REQUIREMENTS_UPDATE_TOOL.resultSchema>(guidanceData);
