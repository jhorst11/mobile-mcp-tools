...
import {
  FEATURE_BRIEF_REVIEW_RESULT_SCHEMA,
  type FeatureBriefModification,
} from '../../../tools/prd/shared/featureBriefSchemas.js';
...
export class PRDFeatureBriefReviewNode extends AbstractGuidanceNode<PRDState> {
  ...
  execute = (state: PRDState): Partial<PRDState> => {
    ...
    const resultSchema = FEATURE_BRIEF_REVIEW_RESULT_SCHEMA;
    ...
    const guidanceData: NodeGuidanceData = {
      ...
      metadata: {
        nodeName: this.name,
        description: 'Review the feature brief with the user',
      },
    };

    const validatedResult = this.executeWithGuidance(guidanceData) as z.infer<
  ...
  private processReviewResult(
    validatedResult: { approved: boolean; modifications?: FeatureBriefModification[] },
    _state: PRDState
  ): Partial<PRDState> {
